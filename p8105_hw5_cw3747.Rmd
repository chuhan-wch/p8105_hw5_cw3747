...
The sample average of $\hat{\mu}$ among tests that reject $H_0$ is not approximately equal to the true $\mu$. We can see from the plot, when the $\mu$ is around 1, conditioning on significance selects extreme estimates, so the $\hat{\mu}$ shifted away from 0. This may due to a form of selection bias. As $\mu$ becomes large, almost all samples reject and the conditioning disappears, so that almost of them reject $H_0$, $\mu$ and $\hat{\mu}$ are close.
...
plot_df <- city_prop_all |>
  mutate(
    city_state = fct_reorder(city_state, p_hat)
  )

ggplot(plot_df, aes(x = city_state, y = p_hat)) +
...
The plot ranks cities by the share of homicides that are unsolved, with points and 95% CIs. Rates are highest in Chicago (~74%), New Orleans and Baltimore (~65%), while several cities sit near 0–10% at the low end; most fall around 40–60%. Wider intervals reflect smaller case counts, highlighting uncertainty for some cities.