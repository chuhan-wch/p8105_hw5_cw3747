...
estimates, so the $\hat{\mu}$ shifted away from 0. This may due to a
form of selection bias. As $\mu$ becomes large, almost all samples
reject and the conditioning disappears, so that almost of them reject
$H_0$, $\mu$ and $\hat{\mu}$ are close.
...
<img src="p8105_hw5_cw3747_files/figure-gfm/unnamed-chunk-17-1.png" width="90%" />

The plot ranks cities by the share of homicides that are unsolved, with
points and 95% CIs. Rates are highest in Chicago (~74%), New Orleans and
Baltimore (~65%), while several cities sit near 0–10% at the low end;
most fall around 40–60%. Wider intervals reflect smaller case counts,
highlighting uncertainty for some cities.
